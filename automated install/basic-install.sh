...
os_check() {
    if [ "$PIHOLE_SKIP_OS_CHECK" != true ]; then
        ...
        local remote_os_domain valid_os valid_version valid_response detected_os detected_version display_warning
        ...
        if [ "$display_warning" != false ]; then
            if [ "$valid_response" = false ]; then
                printf "  %b %bRetrieval of supported OS list failed. Reason: dig failure. %b\\n" "${CROSS}" "${COL_LIGHT_RED}" "${COL_NC}"
                printf "      %bUnable to determine if the OS (%s %s) is supported%b\\n" "${COL_LIGHT_RED}" "${detected_os^}" "${detected_version}" "${COL_NC}"
            else
            ...
            printf "\\n"
            printf "      If you wish to attempt an install anyway, you can try one of the following commands to skip this check:\\n"
            printf "\\n"
